...
class VOC12Dataset(Dataset):
    def __init__(self, img_name_list_path, voc12_root, train=True, transform=None,transform_l=None, gen_attn=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform:
            img = self.transform(img)
            img_crop = self.transform_l(img_crop)
        return img, img_crop, label
...
def build_dataset(is_train, args, gen_attn=False):
    ...
    transform_l = build_transform(is_train, args,large=True)
    dataset = None
    nb_classes = None

    if args.data_set == 'VOC12':
        dataset = VOC12Dataset(img_name_list_path=args.img_list, voc12_root=args.data_path,
                               train=is_train, gen_attn=gen_attn, transform=transform,transform_l=transform_l)
        nb_classes = 20
...
def build_transform(is_train, args,large = False):
    if large == True:
