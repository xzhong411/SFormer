...
def get_img_path1(img_name, voc12_root):
    if img_name.endswith('.jpg'):
        return "%s%s"%(voc12_root, img_name)
    else:
        return get_img_path(img_name,voc12_root)
...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img = PIL.Image.open(get_img_path1(name, self.voc12_root)).convert("RGB")
...
class VOC12AffDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        label_la_path = os.path.join(self.label_la_dir, name.split('/')[2].split('.')[0] + '.npy')

        label_ha_path = os.path.join(self.label_ha_dir, name.split('/')[2].split('.')[0] + '.npy')
